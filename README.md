...
- A timer tracks how long the current puzzle has taken — pause it any time (⏸ next to the timer) to stop the clock and hide the grid and word list behind a "Paused" overlay; resuming picks the clock back up exactly where it left off, even across a page reload
