...
- **Bonus words** — dense grids sometimes spell out a real word by chance that wasn't on the list (usually short). Trace one and it's marked as a gold ★ bonus find, checked against the rest of that category's own word lists rather than a separate dictionary
