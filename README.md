...
- **Category picker: Random or Select**, in the "⋮" menu. Random (default) picks a category and topic automatically each time you start a new puzzle. Select opens a tree list — tap a category name for a mixed puzzle from all its topics, or tap ▸ to expand it and pick one topic specifically — with the difficulty picker right there too, so both are set in one place before the puzzle generates. Reopening the picker mid-game highlights and auto-expands whichever category/topic the current puzzle is actually using
- **Difficulty filter** — Easy / Moderate / Hard, shown as a colored badge next to the title; also in the "⋮" menu. Grid size (roughly 7×7 up to 14×14, depending on difficulty) is sized to the chosen word-set so puzzles stay dense rather than sparse. Difficulty also caps how many words may cross through a single cell — 2 on Easy, 3 on Moderate, 4 on Hard — so harder puzzles get more tangled, not just bigger
- Each set mixes short (3–4 letter) and long (7+ letter) words, and words are placed to cross through shared letters wherever possible — grids are dense, not a handful of same-length words lost in empty space. A cell shared by multiple words is split into that many equal color wedges, so a triple or quadruple crossing looks visually distinct from a simple two-way one
- **Bonus words** — 2–3 of the words already on the list secretly appear a second time elsewhere in the grid. Find that second occurrence and the word's chip is marked with a gold ★, on top of the normal find. The header's found-count badge shows bonus progress too whenever a puzzle has any (e.g. `3/9 ★1/2`)
