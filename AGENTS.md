...
- **`words.en.json`** / **`words.uk.json`** — `{ "categories": { "animals": { "sets": [ { "words": [...] }, ... ] }, ... } }`, one file per language, 11 categories (`animals`, `countries`, `food`, `colors`, `sports`, `space`, `weather`, `jobs`, `vehicles`, `clothing`, `music`). Each category holds 2–3 curated **sets** (e.g. animals splits into "Farm & Pets" / "Wild & Exotic" / "Sea, Air & Reptiles") instead of one flat pool — `pickWordSet()` in `script.js` picks one whole set at random per puzzle, so a single game pulls from a thematically coherent group rather than the entire category. Sets aren't named/exposed anywhere in the UI, just internal groupings (the theme names above are documentation only, not stored in the JSON). Words are lowercase, single alphabetic tokens (no spaces/hyphens), 3–16 letters (16 = the hard grid's max dimension), and each set mixes short (3–4 letter) and long (7+ letter) words — a set of same-length words makes the puzzle trivially easy regardless of grid size. If you add a category or a set, keep that length spread and aim for at least ~8 words per set (see the dynamic sizing note below for why); add the category's id to `CATEGORIES` in `script.js`, its sets to both language files, and the matching `category<Id>` key to both `i18n.js` language objects — `renderMenus()` looks up `category${capitalize(id)}` and will render `undefined` if the key is missing.
